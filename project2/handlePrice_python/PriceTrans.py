...
def convert_excel_to_csv(input_file, output_file):
    ...
    result_df = pd.DataFrame(columns=['x_station', 'y_station', 'price'])
        ...
        for j in range(3, len(df.columns)):
            ...
            if not pd.isnull(price):
                ...
                if not ((result_df['x_station'] == x_station) & (result_df['y_station'] == y_station)).any():
                    result_df = pd.concat(
                        [result_df, pd.DataFrame({'x_station': [x_station], 'y_station': [y_station], 'price': [price]})],
                        ignore_index=True)
...
if __name__ == "__main__":
    convert_excel_to_csv(r"D:\Program Files\PyCharm 2023.3.3\HappyTry\Price.xlsx", "priceOut.csv")
